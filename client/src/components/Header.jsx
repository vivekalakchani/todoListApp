...
const Header = () => {
  ...
  const handleLogout = async () => {
    try {
      await logoutUser(); // Assuming this function makes an API call to logout
      dispatch(clearUser());
      navigate("/login");
    } catch (error) {
    ...
    <header className="bg-indigo-500 text-white p-4 shadow-md">
      <nav className="flex justify-between items-center">
        ...
        <ul className="flex space-x-4">
          ...
          <li>
            <button
              className="bg-red-500 text-white px-6 py-2 rounded-lg shadow-lg hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 flex items-center space-x-2"
              type="button"
